...
@Configuration
@EnableKafka
public class KafkaConsumerSemiAutoCommitConfiguration {
...
//        props.put(ConsumerConfig.HEARTBEAT_INTERVAL_MS_CONFIG,) //心跳间隔
    ...
    /**
     * 消费者并发批量消费配置-由spring-kafka半自动提交offset
     */
    @Bean("semiAutoCommitContainerFactory")
    public KafkaListenerContainerFactory<?> semiAutoCommitContainerFactory() {
...
//        factory.setConcurrency(Runtime.getRuntime().availableProcessors());
        ...
        //BatchMessagingMessageListenerAdapter，否则创建的是MessagingMessageListenerAdapter
        factory.setBatchListener(true); //如果设置为true，是不支持AckMode.RECORD的
        //AckMode.Batch-每次poll之后一次性提交offset，其他说明参考有道云笔记
